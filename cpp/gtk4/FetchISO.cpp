...
public:
	...
	set_default_size(FETCHISO_WIDTH, FETCHISO_HEIGHT);
...
protected:
	...
	void start_download(const ISOEntry& iso) {
		...
		dialog->set_default_size(FETCHISO_DIALOG_WIDTH, FETCHISO_DIALOG_HEIGHT);
	...
	void perform_download(const ISOEntry& iso) {
		...
		if (dl.downloadFile(iso.url, iso.file)) {
			...
			success->set_default_size(FETCHISO_DIALOG_WIDTH, FETCHISO_DIALOG_HEIGHT);
			...
			failure->set_default_size(FETCHISO_DIALOG_WIDTH, FETCHISO_DIALOG_HEIGHT);
