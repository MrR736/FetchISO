...
void draw_scrollbar(int start_y, int list_size, int visible_items, int selected) {
    if (list_size <= visible_items) return;

    int max_scrollbar_height = visible_items;
    int scrollbar_height = (visible_items * visible_items) / list_size;
    if (scrollbar_height < 1) scrollbar_height = 1;

    int scroll_pos = (selected * (max_scrollbar_height - scrollbar_height)) / (list_size - 1);

    for (int i = 0; i < max_scrollbar_height; i++) {
        if (i >= scroll_pos && i < scroll_pos + scrollbar_height)
            mvprintw(start_y + i, COLS - 2, "█");
        else
            mvprintw(start_y + i, COLS - 2, "|");
    }
}
...
int main() {
    ...
    while (choice == -1) {
        ...
        mvprintw(2, 2, "Use ↑/↓ to navigate, Enter to select, Q to quit");

        if (selecting_arch) {
            for (size_t i = 0; i < arch_keys.size(); i++) {
                if ((int)i == arch_index) attron(A_REVERSE);
                mvprintw(4 + i, 4, arch_map[arch_keys[i]].c_str());
                attroff(A_REVERSE);
            }
            refresh();
            ch = getch();
        ...
        for (int i = 0; i < visible_items && first_visible + i < (int)filtered_list.size(); i++) {
            ...
            mvprintw(i + 4, 4, filtered_list[first_visible + i].name.c_str());
            attroff(A_REVERSE);
        }
    ...
    clear();
    mvprintw(5, 5, "Downloading: %s", filtered_list[choice].name.c_str());
    refresh();
    ...
    if (downloader.downloadFile(filtered_list[choice].url, download_path)) {
        mvprintw(7, 5, "Download complete: %s", download_path.c_str());
    } else {
