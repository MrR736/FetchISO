...
public:
    FetchISO(QWidget *parent = nullptr) : QWidget(parent) {
        ...
        for (const auto& [key, value] : archMap) {
...
private slots:
    void updateSearchResults() {
        ...
        for (const auto &iso : isoList) {
            ...
            bool matchesArch = (selectedArch == "All Architectures" ||
                                (archMap.count(iso.arch) && archMap[iso.arch] == selectedArch.toStdString()));
    ...
    void startDownload(QListWidgetItem *item) {
        ...
        reply = QMessageBox::question(this, "Download Confirmation",
                                      "Do you want to download: " + isoName +
                                      "\nFrom: " + isoUrl +
                                      "\n\nFile: " + isoFile,
...
private:
    ...
    std::unordered_map<std::string, std::string> archMap = {
        ...
        for (const auto &entry : releases) {
            try {
                isoList.push_back({
                    ...
                    entry.at("arch").get<std::string>(),
                    entry.at("file").get<std::string>()
                });
            } catch (const std::exception &e) {
                std::cerr << "Skipping invalid entry: " << e.what() << std::endl;
            }
        }
    }
};
